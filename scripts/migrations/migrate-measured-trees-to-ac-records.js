#!/usr/bin/env node
/*
  Migrate measured tree photos from S3/PDS GeoJSON blobs to app.gainforest.ac.multimedia records.

  For each org with tree GeoJSON data:
    1. List site records and fetch tree GeoJSON (from PDS blob or S3 fallback)
    2. For each tree feature, ensure a dwc.occurrence record exists (idempotent via deterministic rkey)
       NOTE: occurrence records do NOT get trunkEvidence/leafEvidence/barkEvidence fields
    3. For each photo (trunk, leaf, bark), create a separate app.gainforest.ac.multimedia record:
       - uploadBlob → createRecord (atomic pattern, prevents blob GC)
    4. Create dwc.measurement records for DBH and height

  Resumability: progress tracked in scripts/migrations/data/ac-measured-trees-progress.json
  Errors logged to: scripts/migrations/data/ac-measured-trees-errors.json

  Usage:
    node scripts/migrations/migrate-measured-trees-to-ac-records.js \
      ...
      [--limit N] \
      [--dry-run] \
      [--out tmp/ac-measured-trees-migration-results.json]
*/
...
const PROGRESS_FILE = 'scripts/migrations/data/ac-measured-trees-progress.json'
const ERRORS_FILE = 'scripts/migrations/data/ac-measured-trees-errors.json'
...
function loadCredentials(file) {
  if (!fs.existsSync(file)) {
    console.warn(`[ac-measured-trees] WARNING: Credentials file not found: ${file}`)
    return new Map()
  }
  const raw = JSON.parse(fs.readFileSync(file, 'utf8'))
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        ...
        console.warn(`[ac-measured-trees]   Download returned null (attempt ${attempt}/${maxRetries}), retrying in ${delay}ms...`)
        await sleep(delay)
      }
    } catch (err) {
      if (attempt < maxRetries) {
        const delay = Math.pow(2, attempt) * 500
        console.warn(`[ac-measured-trees]   Download error (attempt ${attempt}/${maxRetries}): ${err?.message || String(err)}, retrying in ${delay}ms...`)
        await sleep(delay)
      } else {
        console.warn(`[ac-measured-trees]   Download failed after ${maxRetries} attempts: ${err?.message || String(err)}`)
      }
    }
  }
  return null
...
async function getSharp() {
  ...
  try {
    ...
    console.warn('[ac-measured-trees] WARNING: sharp module not available. Images will be uploaded uncompressed.')
    return null
  }
}
...
async function compressImage(buffer) {
  ...
  try {
    ...
    console.warn(`[ac-measured-trees]   Image compression failed: ${err?.message || String(err)}, using original`)
    return { buffer, mimeType: 'image/jpeg' }
  }
}
...
async function listExistingOccurrenceRkeys(agent, did) {
  ...
  try {
    ...
    console.warn(`[ac-measured-trees] Warning: failed to list existing occurrences for ${did}: ${err?.message || String(err)}`)
  }
  return rkeys
}
...
async function listExistingMeasurementRkeys(agent, did) {
  ...
  try {
    ...
    console.warn(`[ac-measured-trees] Warning: failed to list existing measurements for ${did}: ${err?.message || String(err)}`)
  }
  return rkeys
}
...
async function listExistingAcMultimediaRkeys(agent, did) {
  ...
  try {
    ...
    console.warn(`[ac-measured-trees] Warning: failed to list existing AC multimedia records for ${did}: ${err?.message || String(err)}`)
  }
  return rkeys
}
...
/**
 * Process a single photo URL: download, compress, upload blob, create AC multimedia record.
 * This is the atomic pattern: uploadBlob → createRecord (prevents blob GC).
 * Returns true on success, false on failure, 'skipped' if URL is a Google Drive URL.
 * @param {import('@atproto/api').AtpAgent} agent
 * @param {string} did
 * @param {string} url
 * @param {string} subjectPart  AC subjectPart value (e.g. 'entireOrganism', 'leaf', 'bark')
 * @param {string} occurrenceAtUri  AT-URI of the linked dwc.occurrence record
 * @param {string} acRkey  Deterministic rkey for the AC record
 * @param {string} tmpDir  Temp directory for intermediate files
 * @param {string} nowIso  ISO timestamp for createdAt
 * @returns {Promise<'created' | 'skipped' | 'failed'>}
 */
async function processPhotoAsAcRecord(agent, did, url, subjectPart, occurrenceAtUri, acRkey, tmpDir, nowIso) {
  if (!url) return 'skipped'

  // Check for Google Drive URLs (Oceanus) — skip
  if (isGoogleDriveUrl(url)) {
    console.log(`[ac-measured-trees]     Skipping Google Drive URL (${subjectPart}): ${url}`)
    return 'skipped'
  }

  // Download
  console.log(`[ac-measured-trees]     Downloading ${subjectPart}: ${url.slice(0, 80)}...`)
  const downloaded = await fetchBinaryWithRetry(url)
  if (!downloaded) {
    console.warn(`[ac-measured-trees]     Download failed for ${subjectPart}: ${url}`)
    return 'failed'
  }

  // Write to temp file (for disk safety — one at a time)
  ...
  try {
    ...
    // Step 1: Upload blob to PDS
    const uploadRes = await agent.uploadBlob(compressed, { encoding: mimeType })
    const blobRef = uploadRes.data.blob

    console.log(`[ac-measured-trees]     Uploaded ${subjectPart}: ${(compressed.length / 1024).toFixed(1)}KB WebP`)

    // Step 2: Immediately create the AC multimedia record referencing the blob
    // This is the atomic pattern — blob is referenced before it can be GC'd
    const acRecord = {
    ...
    console.log(`[ac-measured-trees]     Created AC multimedia record: ${acRkey} (${subjectPart})`)
    return 'created'
  } finally {
...
async function main() {
  ...
  console.log('[ac-measured-trees] Starting migration', {
  ...
  console.log(`[ac-measured-trees] Loaded ${repos.length} repos from inventory`)

  // Load credentials
  const credentials = loadCredentials(args.credentials)
  console.log(`[ac-measured-trees] Loaded credentials for ${credentials.size} orgs`)

  // Load progress
  const progress = loadProgress()
  console.log(`[ac-measured-trees] Loaded progress for ${Object.keys(progress).length} orgs`)

  // Load errors
  const errors = loadErrors()
  console.log(`[ac-measured-trees] Loaded ${errors.length} existing errors`)
  ...
  console.log(`[ac-measured-trees] Processing ${targets.length} repos`)

  const agent = new AtpAgent({ service: args.service })
  const nowIso = new Date().toISOString()

  // Create a temp directory for image processing
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ac-measured-trees-'))
  console.log(`[ac-measured-trees] Using temp dir: ${tmpDir}`)
  ...
  try {
    for (const repo of targets) {
      ...
      console.log(`\n[ac-measured-trees] === Processing ${handle} (${did}) ===`)

      // Get credentials
      const password = credentials.get(handle)
      if (!password) {
        console.log(`[ac-measured-trees] Skipping ${handle}: no credentials`)
      ...
      if (!args.dryRun) {
        try {
          ...
          console.log(`[ac-measured-trees] Logged in as ${handle}`)
        } catch (err) {
          console.error(`[ac-measured-trees] Login failed for ${handle}: ${err?.message || String(err)}`)
      ...
      if (!args.dryRun) {
        try {
          ...
          console.log(`[ac-measured-trees] Found ${siteRecords.length} site records for ${handle}`)
        } catch (err) {
          console.error(`[ac-measured-trees] Failed to list sites for ${handle}: ${err?.message || String(err)}`)
      ...
      // Phase 1b: Fetch existing occurrence/measurement/AC rkeys for idempotency
      ...
      if (!args.dryRun) {
        ...
        existingAcMultimediaRkeys = await listExistingAcMultimediaRkeys(agent, did)
        console.log(`[ac-measured-trees] Existing occurrences: ${existingOccurrenceRkeys.size}, measurements: ${existingMeasurementRkeys.size}, AC multimedia: ${existingAcMultimediaRkeys.size}`)
      }
      ...
      for (const siteRecord of sitesToProcess) {
        ...
        console.log(`[ac-measured-trees]  Site: ${siteRkey} (${siteRecord.value?.name || 'unnamed'})`)
        ...
        if (treesBlob && !args.dryRun) {
          ...
          if (blobCid) {
            console.log(`[ac-measured-trees]   Fetching tree GeoJSON from PDS blob: ${blobCid}`)
            try {
              ...
              if (blobData && blobData.buffer.length > 100) {
                ...
                console.log(`[ac-measured-trees]   Loaded ${geojson?.features?.length || 0} features from PDS blob`)
              }
            } catch (err) {
              console.warn(`[ac-measured-trees]   PDS blob fetch failed: ${err?.message || String(err)}`)
            }
          }
        }
        ...
        if (!geojson || !Array.isArray(geojson.features) || geojson.features.length === 0) {
          ...
          console.log(`[ac-measured-trees]   Falling back to S3: ${s3Url}`)
          if (!args.dryRun) {
            try {
              geojson = await fetchJson(s3Url)
              if (geojson && Array.isArray(geojson.features)) {
                console.log(`[ac-measured-trees]   Loaded ${geojson.features.length} features from S3`)
              } else {
                console.log(`[ac-measured-trees]   No valid GeoJSON from S3 for ${handle}`)
                geojson = null
              }
            } catch (err) {
              console.warn(`[ac-measured-trees]   S3 fetch failed: ${err?.message || String(err)}`)
              geojson = null
            }
          } else {
            console.log(`[ac-measured-trees]   [DRY RUN] Would fetch from S3: ${s3Url}`)
          }
        }

        if (!geojson || !Array.isArray(geojson.features) || geojson.features.length === 0) {
          if (!args.dryRun) {
            console.log(`[ac-measured-trees]   No tree features found for site ${siteRkey}, skipping`)
            continue
          }
        }
        ...
        for (let featureIdx = 0; featureIdx < features.length; featureIdx++) {
          ...
          // Build deterministic rkey for occurrence
          ...
          // Build occurrence record — NO evidence blob fields (those go in AC records)
          ...
          if (args.dryRun) {
            console.log(`[ac-measured-trees]   [DRY RUN] Would create occurrence: ${occurrenceRkey} (${species} @ ${lat},${lon})`)
            if (trunkUrl && !isGoogleDriveUrl(trunkUrl)) {
              const acRkey = makeAcMultimediaRkey(occurrenceRkey, 'entireOrganism')
              console.log(`[ac-measured-trees]   [DRY RUN] Would create AC record: ${acRkey} (entireOrganism)`)
            }
            if (leafUrl && !isGoogleDriveUrl(leafUrl)) {
              const acRkey = makeAcMultimediaRkey(occurrenceRkey, 'leaf')
              console.log(`[ac-measured-trees]   [DRY RUN] Would create AC record: ${acRkey} (leaf)`)
            }
            if (barkUrl && !isGoogleDriveUrl(barkUrl)) {
              const acRkey = makeAcMultimediaRkey(occurrenceRkey, 'bark')
          ...
          // AT-URI for the occurrence record (used as occurrenceRef in AC records)
          const occurrenceAtUri = `at://${did}/${DWC_OCCURRENCE_COLLECTION}/${occurrenceRkey}`

          // Phase 2a: Ensure occurrence record exists (create if not)
          const occurrenceAlreadyExists = existingOccurrenceRkeys.has(occurrenceRkey)
          ...
          if (!occurrenceAlreadyExists) {
            try {
              ...
              orgOccurrencesCreated++
              console.log(`[ac-measured-trees]   Created occurrence: ${occurrenceRkey} (${species})`)
            } catch (err) {
              console.error(`[ac-measured-trees]   Failed to create occurrence ${occurrenceRkey}: ${err?.message || String(err)}`)
            ...
            orgOccurrencesSkipped++
            console.log(`[ac-measured-trees]   Occurrence already exists: ${occurrenceRkey}`)
          }

          // Phase 2b: Create AC multimedia records for each photo
          // Each photo: uploadBlob → createRecord (atomic, prevents blob GC)

          // Trunk photo → subjectPart: 'entireOrganism'
          if (trunkUrl) {
            const acRkey = makeAcMultimediaRkey(occurrenceRkey, 'entireOrganism')
            if (!existingAcMultimediaRkeys.has(acRkey)) {
              try {
                const result = await processPhotoAsAcRecord(
                  agent, did, trunkUrl, 'entireOrganism',
                  occurrenceAtUri, acRkey, tmpDir, nowIso
                )
                if (result === 'created') {
                  existingAcMultimediaRkeys.add(acRkey)
                  orgAcRecordsCreated++
                  ...
                  appendError(errors, { handle, did, siteRkey, featureIdx, photoType: 'trunk', phase: 'create-ac-record', url: trunkUrl })
                  orgErrors++
                }
              } catch (err) {
                console.warn(`[ac-measured-trees]   Trunk AC record creation failed: ${err?.message || String(err)}`)
                appendError(errors, { handle, did, siteRkey, featureIdx, photoType: 'trunk', error: err?.message || String(err), url: trunkUrl })
                orgErrors++
              }
            } else {
              console.log(`[ac-measured-trees]   AC record already exists: ${acRkey} (entireOrganism)`)
            }
          }

          // Leaf photo → subjectPart: 'leaf'
          if (leafUrl) {
            const acRkey = makeAcMultimediaRkey(occurrenceRkey, 'leaf')
            if (!existingAcMultimediaRkeys.has(acRkey)) {
              try {
                const result = await processPhotoAsAcRecord(
                  agent, did, leafUrl, 'leaf',
                  occurrenceAtUri, acRkey, tmpDir, nowIso
                )
                if (result === 'created') {
                  existingAcMultimediaRkeys.add(acRkey)
                  ...
                  appendError(errors, { handle, did, siteRkey, featureIdx, photoType: 'leaf', phase: 'create-ac-record', url: leafUrl })
                  orgErrors++
                }
              } catch (err) {
                console.warn(`[ac-measured-trees]   Leaf AC record creation failed: ${err?.message || String(err)}`)
                appendError(errors, { handle, did, siteRkey, featureIdx, photoType: 'leaf', error: err?.message || String(err), url: leafUrl })
                orgErrors++
              }
            } else {
              console.log(`[ac-measured-trees]   AC record already exists: ${acRkey} (leaf)`)
            }
          }

          // Bark photo → subjectPart: 'bark'
          if (barkUrl) {
            const acRkey = makeAcMultimediaRkey(occurrenceRkey, 'bark')
            if (!existingAcMultimediaRkeys.has(acRkey)) {
              try {
                const result = await processPhotoAsAcRecord(
                  agent, did, barkUrl, 'bark',
                  occurrenceAtUri, acRkey, tmpDir, nowIso
                )
                if (result === 'created') {
                  existingAcMultimediaRkeys.add(acRkey)
                  ...
                  appendError(errors, { handle, did, siteRkey, featureIdx, photoType: 'bark', phase: 'create-ac-record', url: barkUrl })
                  orgErrors++
                }
              } catch (err) {
                console.warn(`[ac-measured-trees]   Bark AC record creation failed: ${err?.message || String(err)}`)
                appendError(errors, { handle, did, siteRkey, featureIdx, photoType: 'bark', error: err?.message || String(err), url: barkUrl })
                orgErrors++
              }
            } else {
              console.log(`[ac-measured-trees]   AC record already exists: ${acRkey} (bark)`)
            }
          }

          // Phase 3: Create measurement records for DBH and height
          if (dbh !== null) {
            ...
            if (!existingMeasurementRkeys.has(dbhRkey)) {
              ...
              try {
                ...
                console.warn(`[ac-measured-trees]   Failed to create DBH measurement: ${err?.message || String(err)}`)
          ...
          if (height !== null) {
            ...
            if (!existingMeasurementRkeys.has(heightRkey)) {
              ...
              try {
                ...
                console.warn(`[ac-measured-trees]   Failed to create height measurement: ${err?.message || String(err)}`)
      ...
      console.log(`[ac-measured-trees] ${handle}: occurrences created=${orgOccurrencesCreated} skipped=${orgOccurrencesSkipped} measurements=${orgMeasurementsCreated} acRecords=${orgAcRecordsCreated} errors=${orgErrors}`)

      results.push({
        handle,
        did,
        status: orgErrors === 0 ? 'success' : orgOccurrencesCreated > 0 || orgAcRecordsCreated > 0 ? 'partial' : 'failed',
        ...
        acRecordsCreated: orgAcRecordsCreated,
        errors: orgErrors
      })
  ...
  const summary = {
    ...
    totalAcRecordsCreated: results.reduce((s, r) => s + (r.acRecordsCreated || 0), 0),
    stats: {
  ...
  console.log('\n[ac-measured-trees] Summary:', summary)

  const output = { summary, results }
  fs.mkdirSync(path.dirname(args.out), { recursive: true })
  fs.writeFileSync(args.out, JSON.stringify(output, null, 2))
  console.log(`[ac-measured-trees] Wrote results to ${args.out}`)
  console.log(`[ac-measured-trees] Progress saved to ${PROGRESS_FILE}`)
  console.log(`[ac-measured-trees] Errors saved to ${ERRORS_FILE}`)
}

main().catch((err) => {
  console.error('[ac-measured-trees] Fatal error:', err)
  process.exit(1)
})
